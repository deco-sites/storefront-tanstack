...
const SIMULATE_PRESETS: Record<string, { regionCode: string; city: string; country: string; lat?: string; lng?: string }> = {
  ...
  // SC = Santa Catarina (Florianópolis). DDD da região é 47/48, mas a Cloudflare
  // devolve a sigla ISO "SC" — nunca o DDD. Esse preset prova que a regra
  // `regionCode: "47"` NÃO casa com alguém em SC (parity com deco-cx/apps).
...
const inspectGeo = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<GeoSnapshot> => {
    ...
    // ?simulate=SP|RJ|MG|47 — bypasses real geo (workerd injects real CF data
    // locally, so we need an explicit override path for testing).
    ...
    const matcherCtx = {
      ...
      // When simulating, drop request to skip request.cf — cookies win.
    ...
    const rules: Array<{ label: string; rule: any }> = [
      {
        label: "include SP",
        rule: { includeLocations: [{ regionCode: "SP" }] },
      },
      {
        label: "include RJ",
        rule: { includeLocations: [{ regionCode: "RJ" }] },
      },
      {
        label: "include '47' (raw)",
        rule: { includeLocations: [{ regionCode: "47" }] },
      },
      {
        label: "include country=BR",
        rule: { includeLocations: [{ country: "BR" }] },
      },
      {
        label: "include country='Brasil' (alias)",
        rule: { includeLocations: [{ country: "Brasil" }] },
      },
      {
        label: "include SP, exclude RJ",
        rule: {
          includeLocations: [{ regionCode: "SP" }],
          excludeLocations: [{ regionCode: "RJ" }],
        },
      },
      {
        label: "empty includeLocations: [{}] (parity: matches all)",
        rule: { includeLocations: [{}] },
      },
      {
        label: "haversine SP center 50km",
        rule: {
          includeLocations: [{ coordinates: "-23.5505,-46.6333,50000" }],
        },
      },
      {
        label: "haversine Rio center 50km",
        rule: {
          includeLocations: [{ coordinates: "-22.9068,-43.1729,50000" }],
        },
      },
    ];

    // Probe the framework's resolved geo through the patched matcher.
    // We call evaluateMatcher with a "passthrough" rule to inspect what the
    // bundle sees — but the actual geo extraction is what evaluateMatcher
    // uses internally. So we read geo directly from the same primitives
    // the bundled getGeoData would read.
    ...
    // When simulating, mirror the matcher's view: ignore real cf-* headers
    // and request.cf, read only from the overridden cookies. (Workerd injects
    // real geo headers locally based on the IP, which would mask the simulate.)
...
function GeoDebug() {
  ...
  return (
    ...
    >
      {/* Visual segmentation demo — what the C&V home would show today */}
...
function RegionGreeting({ source }: { source: GeoSnapshot["source"] }) {
  // Same logic the CMS would use, via the same matcher
